...
//For Testing Only
// File @openzeppelin/contracts/token/ERC721/IERC721Receiver.sol@v3.4.1-solc-0.7-2
...
// File @uniswap/v3-core/contracts/interfaces/IUniswapV3Factory.sol@v1.0.0
...
// File @uniswap/v3-core/contracts/interfaces/IERC20Minimal.sol@v1.0.0
...
// File @uniswap/v3-periphery/contracts/interfaces/IPoolInitializer.sol@v1.0.1
...
// File @uniswap/v3-periphery/contracts/interfaces/IERC721Permit.sol@v1.0.1
...
// File @uniswap/v3-periphery/contracts/interfaces/IPeripheryPayments.sol@v1.0.1
...
// File @uniswap/v3-periphery/contracts/interfaces/IPeripheryImmutableState.sol@v1.0.1
...
// File @uniswap/v3-periphery/contracts/libraries/PoolAddress.sol@v1.0.1
...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x68b58c0c7cb7eee735ff41e28d34d055e34ebf55c6170f58890d725e87826579;
...
// File @uniswap/v3-periphery/contracts/interfaces/INonfungiblePositionManager.sol@v1.0.1
...
// File @uniswap/v3-periphery/contracts/interfaces/IMulticall.sol@v1.0.1
...
// File contracts/interfaces/IUniswapV3Staker.sol
...
// File contracts/libraries/IncentiveId.sol
...
// File contracts/libraries/RewardMath.sol
...
// File contracts/libraries/NFTPositionInfo.sol
...
// File @uniswap/v3-periphery/contracts/libraries/TransferHelper.sol@v1.0.1
...
// File @uniswap/v3-periphery/contracts/base/Multicall.sol@v1.0.1
...
// File contracts/HaydenV3Staker.sol
...
contract HaydenV3Staker is IUniswapV3Staker, Multicall {
    /// @notice Represents a staking incentive
    struct Incentive {
        uint256 totalRewardUnclaimed;
        uint160 totalSecondsClaimedX128;
    ...
    function createIncentive(IncentiveKey memory key, uint256 reward) external override {
        require(reward > 0, 'HaydenV3Staker::createIncentive: reward must be positive');
        require(
            block.timestamp <= key.startTime,
            'HaydenV3Staker::createIncentive: start time must be now or in the future'
        );
        require(
            key.startTime - block.timestamp <= maxIncentiveStartLeadTime,
            'HaydenV3Staker::createIncentive: start time too far into future'
        );
        require(key.startTime < key.endTime, 'HaydenV3Staker::createIncentive: start time must be before end time');
        require(
            key.endTime - key.startTime <= maxIncentiveDuration,
            'HaydenV3Staker::createIncentive: incentive duration is too long'
        );
    ...
    function endIncentive(IncentiveKey memory key) external override returns (uint256 refund) {
        require(block.timestamp >= key.endTime, 'HaydenV3Staker::endIncentive: cannot end incentive before end time');
        ...
        require(refund > 0, 'HaydenV3Staker::endIncentive: no refund available');
        require(
            incentive.numberOfStakes == 0,
            'HaydenV3Staker::endIncentive: cannot end incentive while deposits are staked'
        );
    ...
    function onERC721Received(
        ...
        require(
            ...
            'HaydenV3Staker::onERC721Received: not a univ3 nft'
        );
    ...
    function transferDeposit(uint256 tokenId, address to) external override {
        require(to != address(0), 'HaydenV3Staker::transferDeposit: invalid transfer recipient');
        address owner = deposits[tokenId].owner;
        require(owner == msg.sender, 'HaydenV3Staker::transferDeposit: can only be called by deposit owner');
    ...
    function withdrawToken(
        ...
        require(to != address(this), 'HaydenV3Staker::withdrawToken: cannot withdraw to staker');
        Deposit memory deposit = deposits[tokenId];
        require(deposit.numberOfStakes == 0, 'HaydenV3Staker::withdrawToken: cannot withdraw token while staked');
        require(deposit.owner == msg.sender, 'HaydenV3Staker::withdrawToken: only owner can withdraw token');
    ...
    function stakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        require(deposits[tokenId].owner == msg.sender, 'HaydenV3Staker::stakeToken: only owner can stake token');
    ...
    function unstakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        ...
        if (block.timestamp < key.endTime) {
            require(
                ...
                'HaydenV3Staker::unstakeToken: only owner can withdraw token before incentive end time'
            );
        }
        ...
        require(liquidity != 0, 'HaydenV3Staker::unstakeToken: stake does not exist');
        ...
        returns (uint256 reward, uint160 secondsInsideX128)
    {
        ...
        (uint160 secondsPerLiquidityInsideInitialX128, uint128 liquidity) = stakes(tokenId, incentiveId);
        require(liquidity > 0, 'HaydenV3Staker::getRewardInfo: stake does not exist');
    ...
    function _stakeToken(IncentiveKey memory key, uint256 tokenId) private {
        require(block.timestamp >= key.startTime, 'HaydenV3Staker::stakeToken: incentive not started');
        require(block.timestamp < key.endTime, 'HaydenV3Staker::stakeToken: incentive ended');

        bytes32 incentiveId = IncentiveId.compute(key);

        require(
            incentives[incentiveId].totalRewardUnclaimed > 0,
            'HaydenV3Staker::stakeToken: non-existent incentive'
        );
        require(
            _stakes[tokenId][incentiveId].liquidityNoOverflow == 0,
            'HaydenV3Staker::stakeToken: token already staked'
        );

        (IUniswapV3Pool pool, int24 tickLower, int24 tickUpper, uint128 liquidity) =
            NFTPositionInfo.getPositionInfo(factory, nonfungiblePositionManager, tokenId);

        require(pool == key.pool, 'HaydenV3Staker::stakeToken: token pool is not the incentive pool');
        require(liquidity > 0, 'HaydenV3Staker::stakeToken: cannot stake token with 0 liquidity');
